...
of a very specific task. 
Integration of multiple samples and specific tasks can be achieved with scripts
and we provide a set to demonstrate the process.

**[bamtocov]({{ site.baseurl }}/{% link _docs/commands/bamtocov.md %})** will produce a _coverage BED_ from a single BAM file, or a count matrix from a set of alignments and a target (in BED, GTF or GFF format).
...
**[bamtocounts]({{ site.baseurl }}/{% link _docs/commands/bamtocounts.md %})** will count the number of reads covering each target region,
rather than the nucleotidic coverage

**[bamcountrefs]({{ site.baseurl }}/{% link _docs/commands/bamcountrefs.md %})** is a shortcut to count the number of reads per chromosome,
with filters on the read flags, length and quality

**[covtotarget]({{ site.baseurl }}/{% link _docs/commands/covtotarget.md %})** (_legacy_) is an utility to create a count table from the _output_ of
